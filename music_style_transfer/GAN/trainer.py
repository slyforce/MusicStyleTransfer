...
class Trainer:
    ...
    def build_gan_models(self):
        self.G, self.vars_G = MIDINetGenerator(
            self.batch_size, MAXIMUM_SEQUENCE_LENGTH, self.data_loader.get_feature_length(), reuse=False)
        self.G_test, _ = MIDINetGenerator(
            1, MAXIMUM_SEQUENCE_LENGTH, self.data_loader.get_feature_length(), reuse=True, is_training=False)

        self.D, self.vars_D = MIDINetDiscriminator(
            self.x, self.data_loader.get_feature_length(), reuse=False)
        self.D_G, _ = MIDINetDiscriminator(
            self.G, self.data_loader.get_feature_length(), reuse=True)

    def build_cgan_models(self):
        self.G, self.vars_G = MIDINetGeneratorSampleInput(
            self.x, None, self.batch_size, MAXIMUM_SEQUENCE_LENGTH, self.data_loader.get_feature_length(), reuse=False)
    ...
    def summary_norm_gradients(self):
        for variable in tf.global_variables():
            if str(
                    variable.name).startswith('Discriminator') or str(
    ...
    def generateTrack(self, iteration):
        ...
        self.save_output_to_file(
            G[0, :, :, 0], self.log_dir + '-' + str(iteration) + ".mid")
    ...
    def save_output_to_file(self, output, filename):
        ...
        self.midi_writer.write_to_file(filename, melody)
