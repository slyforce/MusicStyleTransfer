...
class VariationalKLLoss(mx.gluon.loss.Loss):
    ...
    def hybrid_forward(self, F, z_means, z_vars):
        ...
        kl_loss_dimension_sum = F.sum(kl_loss, axis=1)
        #kl_loss_mean = F.mean(kl_loss_dimension_sum, axis=0)
        return kl_loss_dimension_sum


class SoftmaxCrossEntropy(mx.gluon.loss.SoftmaxCrossEntropyLoss):
    def hybrid_forward(self, F, pred, label, sample_weight=None):
        mask = F.where(label != 0.,
                       F.ones_like(label),
                       F.zeros_like(label))

        pred = F.log(pred)
        loss = -F.pick(pred, label, axis=self._axis, keepdims=True)
        return F.mean(F.squeeze(loss) * F.squeeze(mask), axis=self._batch_axis, exclude=True)
